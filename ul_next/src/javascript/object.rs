...
impl<'a> JSObject<'a> {
    ...
    pub fn new_error(ctx: &'a JSContext, value: JSValue) -> Result<Self, JSValue<'a>> {
        let items = vec![value];
        let items_ptrs: Vec<_> = items.iter().map(|v| v.internal).collect();
        ...
        let result = unsafe {
            ctx.lib.ultralight().JSObjectMakeError(
                ctx.internal,
                items.len() as _,
                items_ptrs.as_ptr(),
                &mut exception,
        ...
        if !exception.is_null() {
            ...
            Err(JSValue::new_string(ctx, "Failed to create error"))
        } else {
            Ok(Self {
                value: JSValue::from_raw(ctx, result),
            })
